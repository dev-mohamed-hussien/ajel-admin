...
import Loader from "components/loading/roller";
type RouteConfig = {
  path: string;
...
function getMetaData(
  ...
  routes
    ...
    .forEach((route) => {
      ...
      const newElement =
        key === "layout"
          ? cloneElement(route.element, { children: <Outlet /> })
          : route.element;
        ...
        childNode = {
          ...
          element: isLastCild ? (
            ...
            <React.Suspense fallback={Loading}>
              {layoutRoutes[part].el}
            </React.Suspense>
          ) : (
            ""
          ),
          children: isLastCild ? undefined : [],
          errorElement: Object.values(errorRoutes).find(
...
export default function collectedRoutes(): RouteObject[] {
  ...
  const mainRoutes = routes.filter(
    (route: RouteConfig): boolean =>
      !route.path.includes("layout") && !route.path.includes("500")
  );
